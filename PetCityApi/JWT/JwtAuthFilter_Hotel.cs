...
{
    public class JwtAuthFilter_Hotel : ActionFilterAttribute  //冒號為繼承  //產生標籤
    {
        // 加解密的 key，如果不一樣會無法成功解密
        private static readonly string secretKey = WebConfigurationManager.AppSettings["TokenKey"];

        /// <summary>
        /// 過濾有用標籤 [JwtAuthFilter_Hotel] 請求的 API 的 JwtToken 狀態及內容
        /// </summary>
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            if (request.Headers.Authorization == null || request.Headers.Authorization.Scheme != "Bearer")
            {
                ...
                string messageJson = JsonConvert.SerializeObject(new { Status = false, Message = "格式不存在且不正確" }); // JwtToken 遺失，需導引重新登入
            ...
            {  //有token 但是跟原本的不一樣
                try
                {
                    ...
                    {
                        string messageJson = JsonConvert.SerializeObject(new { Status = false, Message = "有效期限過期" }); // JwtToken 過期，需導引重新登入
                        var errorMessage = new HttpResponseMessage()
                        {
                            // StatusCode = System.Net.HttpStatusCode.Unauthorized, // 401
                ...
                {
                    // 解密失敗
                    string messageJson = JsonConvert.SerializeObject(new { Status = false, Message = "解密失敗" }); // JwtToken 不符，需導引重新登入
                    var errorMessage = new HttpResponseMessage()
                    {
                        // StatusCode = System.Net.HttpStatusCode.Unauthorized, // 401
